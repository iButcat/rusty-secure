...
use esp32_main::display::{DisplayMessage, LcdDisplay};
...
use esp32_main::tasks::{
    display_task, http_camera_task, http_server_task, led_task, net_runner, sensor_task,
    wifi_connection,
};
...
static DISPLAY_CHANNEL: StaticCell<Channel<CriticalSectionRawMutex, DisplayMessage, 2>> =
    StaticCell::new();
static LED_CHANNEL: StaticCell<Channel<CriticalSectionRawMutex, LedMessage, 1>> = StaticCell::new();
static SENSOR_CHANNEL: StaticCell<Channel<CriticalSectionRawMutex, SensorMessage, 1>> =
    StaticCell::new();
...
static HTTP_CHANNEL: StaticCell<Channel<CriticalSectionRawMutex, HttpMessage, 1>> =
    StaticCell::new();
...
async fn main(spawner: Spawner) {
    ...
    // --- Sensor Task ---
    let trigger = Output::new(peripherals.GPIO5, Level::Low, OutputConfig::default());

    let echo = Input::new(peripherals.GPIO19, InputConfig::default());
    ...
    spawner
        .spawn(sensor_task(sensor, sensor_receiver, display_sender))
        .unwrap();
    ...
    let _wifi_task =
        match spawner.spawn(wifi_connection(wifi_controller_ref, stack, project_config)) {
            Ok(task) => {
    ...
    let http_task = spawner.spawn(http_camera_task(
        stack,
        http_receiver,
        http_sender,
        project_config,
    ...
    let http_server_task = spawner.spawn(http_server_task(&*stack, display_sender));
