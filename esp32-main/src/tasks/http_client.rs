...
use crate::http::client::{ClientError, HttpClient};
...
pub async fn http_camera_task(
    ...
    let state: &mut TcpClientState<2, 1024, 1024> = TCP_CLIENT_STATE.init(TcpClientState::new());
    let tcp_client = TcpClient::new(*stack, state);
    let dns = DnsSocket::new(*stack);
    let mut client = HttpClient::new(&tcp_client, &dns, config.cam_capture_url);
    ...
    loop {
        ...
        match receiver.receive().await {
            HttpMessage::RequestCapture => {
                ...
                match client.request_camera_capture().await {
                    Ok(status_response) => {
                        ...
                        sender
                            .send(HttpMessage::StatusResult(status_response))
                            .await;
                    }
                    Err(e) => {
            ...
            HttpMessage::RequestFailed(client_error) => {
                info!(
                    "RequestFailed({:?}) received in http_camera_task - debugging",
                    client_error
                );
            }
            HttpMessage::StatusResult(result) => {
                info!(
                    "StatusResult({:?}) received in http_camera_task - debugging",
                    result
                );
            }
        }
